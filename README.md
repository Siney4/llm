...
Подними локально LLM-сервер с Qwen2.5-14B-Instruct (см. таблицу выше) и запусти:

- **Linux / macOS** — `./run.sh`
- **Windows** — двойной клик по `run.bat` (или `run.bat` в cmd)

Скрипт сам:

1. найдёт Python ≥ 3.11;
2. создаст `.venv/` и установит зависимости (только при первом запуске или при изменении `pyproject.toml`);
3. при отсутствии `.env` создаст его из `.env.example` и попросит вписать `TELEGRAM_BOT_TOKEN`;
4. запустит бота.
